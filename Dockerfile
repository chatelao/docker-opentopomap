...
# Fill all voids
RUN cd /usr/share/mapnik/osm-custom/hillshade/unpacked && for hgtfile in *.hgt;do gdal_fillnodata.py $hgtfile $hgtfile.tif; done
...
RUN cd /usr/share/mapnik/osm-custom/hillshade/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr 5000 5000 raw.tif warp-5000.tif
RUN cd /usr/share/mapnik/osm-custom/hillshade/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr 1000 1000 raw.tif warp-1000.tif
RUN cd /usr/share/mapnik/osm-custom/hillshade/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr  700  500 raw.tif  warp-700.tif
RUN cd /usr/share/mapnik/osm-custom/hillshade/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr  500  500 raw.tif  warp-500.tif
RUN cd /usr/share/mapnik/osm-custom/hillshade/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr   90   90 raw.tif   warp-90.tif
...
RUN cd /usr/share/mapnik/osm-custom/hillshade/unpacked && gdaldem color-relief -co COMPRESS=LZW -co PREDICTOR=2 -alpha warp-5000.tif relief.txt relief-5000.tif 
RUN cd /usr/share/mapnik/osm-custom/hillshade/unpacked && gdaldem color-relief -co COMPRESS=LZW -co PREDICTOR=2 -alpha warp-500.tif  relief.txt relief-500.tif 
...
RUN cd /usr/share/mapnik/osm-custom/hillshade/unpacked \
...
RUN  cd /usr/share/mapnik/osm-custom/hillshade/unpacked && phyghtmap --max-nodes-per-tile=0 -s 10 -0 --pbf warp-90.tif
