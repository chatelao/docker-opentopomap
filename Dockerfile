...
COPY srtm_list.txt /tmp
RUN cd /tmp && wget -i srtm_list.txt && for zipfile in *.zip;do unzip -j -o "$zipfile" -d unpacked; done

# Fill all voids
RUN cd /tmp/unpacked && for hgtfile in *.hgt;do gdal_fillnodata.py $hgtfile $hgtfile.tif; done


# Merge all .tifs into one huge tif. This file will is the raw DEM with full resolution and the start for any further steps.
# RUN cd /tmp/unpacked && gdal_merge.py -n 32767 -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -o ../../raw.tif *.hgt.tif
RUN cd /tmp/unpacked && gdal_merge.py -n 32767 -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -o raw.tif *.hgt.tif

# Convert the raw file into Mercator projection, interpolate and shrink
RUN cd /tmp/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr 5000 5000 raw.tif warp-5000.tif
RUN cd /tmp/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr 1000 1000 raw.tif warp-1000.tif
RUN cd /tmp/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr  700  500 raw.tif  warp-700.tif
RUN cd /tmp/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr  500  500 raw.tif  warp-500.tif
RUN cd /tmp/unpacked && gdalwarp -co BIGTIFF=YES -co TILED=YES -co COMPRESS=LZW -co PREDICTOR=2 -t_srs "+proj=merc +ellps=sphere +R=6378137 +a=6378137 +units=m" -r bilinear -tr   90   90 raw.tif   warp-90.tif
...
#
COPY relief.txt /tmp/unpacked
# Create color relief for different zoom levels
RUN cd /tmp/unpacked && gdaldem color-relief -co COMPRESS=LZW -co PREDICTOR=2 -alpha warp-5000.tif relief.txt relief-5000.tif 
RUN cd /tmp/unpacked && gdaldem color-relief -co COMPRESS=LZW -co PREDICTOR=2 -alpha warp-500.tif  relief.txt relief-500.tif 
...
RUN apt-get install -y -q \
   ...
   python-numpy	
RUN wget http://katze.tfiu.de/projects/phyghtmap/phyghtmap_1.71.orig.tar.gz \
   && tar -xvzf phyghtmap_1.71.orig.tar.gz \
   && cd phyghtmap-1.71 \
   && python setup.py install
RUN  cd /tmp/unpacked && phyghtmap --max-nodes-per-tile=0 -s 10 -0 --pbf warp-90.tif
